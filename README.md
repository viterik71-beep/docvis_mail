...
- **Перехват ссылок** — все клики по ссылкам в письмах открываются в браузере по умолчанию, а не внутри приложения
...
- При автосохранении вложений опасные файлы автоматически переименовываются: расширение нейтрализуется (`invoice.exe` → `invoice._exe`, `ярлык.lnk` → `ярлык._lnk`), файл невозможно случайно запустить
- При клике на опасный тип файла выводится уведомление **«Опасный файл нейтрализован»**
- Список нейтрализуемых типов: исполняемые файлы (`.exe`, `.msi`, `.com`, `.scr`, `.dll`, `.bat`, `.cmd`), скрипты (`.ps1`, `.vbs`, `.jse`, `.wsf`, `.hta`), ярлыки (`.lnk`, `.url`), реестр (`.reg`), Office-макросы (`.docm`, `.xlsm`, `.pptm` и др.), образы дисков (`.iso`, `.img`), редкие и потенциально опасные архивы (`.jar`, `.tar`, `.gz`, `.bz2`, `.xz` и др.)
- **Сканирование архивов** (ZIP, 7z, RAR): при получении архива программа проверяет список файлов внутри без распаковки — если обнаружены опасные файлы, архив нейтрализуется; при клике на архив результат сканирования показывается пользователю с запросом подтверждения; зашифрованные архивы (содержимое не проверяемо) помечаются отдельно; RAR сканируется через UnRAR.exe (при наличии WinRAR)
